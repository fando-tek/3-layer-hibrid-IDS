...
class Flow:
    """Class ini berfungsi menangkap paket trafik kemudian dikelompokkan berdasarkan IP sumber, IP tujuan, port sumber, port tujuan, dan protokol"""

    def __init__(self, packet: Any, direction: Enum):
        ( self.dest_ip, self.src_ip, self.src_port, self.dest_port, ) = packet_flow_key.get_packet_flow_key(packet, direction)
    ...
    def get_data(self) -> dict:
        """ Fungsi ini untuk mendapatkan nilai fitur yang diekstraksi dari setiap aliran """
        ...
        data = {
            ...
            # Informasi dari waktu paket
            "timestamp": packet_time.get_time_stamp(),
            "flow_duration": int(1e6 * packet_time.get_duration()),
            ...
            "fwd_pkt_len_max": int(packet_length.get_max(PacketDirection.FORWARD)),
            "fwd_pkt_len_min": int(packet_length.get_min(PacketDirection.FORWARD)),
            ...
            "bwd_pkt_len_max": int(packet_length.get_max(PacketDirection.REVERSE)),
            "bwd_pkt_len_min": int(packet_length.get_min(PacketDirection.REVERSE)),
            ...
            # Informasi waktu antar kedatangan paket
            "flow_iat_mean": float(flow_iat["mean"]),
            "flow_iat_max": int(flow_iat["max"]),
            "flow_iat_min": int(flow_iat["min"]),
            "flow_iat_std": float(flow_iat["std"]),
            "fwd_iat_tot": int(forward_iat["total"]),
            "fwd_iat_max": int(forward_iat["max"]),
            "fwd_iat_min": int(forward_iat["min"]),
            "fwd_iat_mean": float(forward_iat["mean"]),
            "fwd_iat_std": float(forward_iat["std"]),
            "bwd_iat_tot": int(backward_iat["total"]),
            "bwd_iat_max": int(backward_iat["max"]),
            "bwd_iat_min": int(backward_iat["min"]),
            ...
            # informasi waktu respon
            "down_up_ratio": int(packet_count.get_down_up_ratio()),
            ...
            "active_max": int(active_stat["max"]),
            "active_min": int(active_stat["min"]),
            "active_mean": float(active_stat["mean"]),
            "active_std": float(active_stat["std"]),
            "idle_max": int(idle_stat["max"]),
            "idle_min": int(idle_stat["min"]),
    ...
    def add_packet(self, packet: Any, direction: Enum) -> None:
        """ Fungsi untuk menambahkan paket ke daftar paket saat ini """
        ...
        # Paket pertama dalam aliran trafik
        if self.start_timestamp == 0:
